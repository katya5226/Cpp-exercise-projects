...
void Competition::readEvents() {
    std::string filename = datapath + "EventList.txt";
    ...
    while (inFile >> number) {
        events.push_back(std::make_shared<Event>(eventNames[number], number));
        numEvents++;
    }
    inFile.close();
}
...
void Competition::printDescending(std::ofstream& log) {
    ...
    log << "Last name" << "\t" << "First name" << "\t" << "Completed" << "\t" << "Weight"
        << "\t" << "Distance" << "\t" << "Time" << "\t" << "Reps" << std::endl;
    for (int i = numCompetitors - 1; i >= 0; i--) {
        log << competitors[i]->name << "\t" << competitors[i]->firstName << "\t1\t0\t0\t0\t0" << std::endl;
    }
}
